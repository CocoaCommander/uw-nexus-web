...
const ProjectListPage = () => {
  ...
  const sortProjects = projects => {
    return projects.sort((a, b) => {
      return (
        new Date(b.duration.created_date) - new Date(a.duration.created_date)
      )
    })
  }
  ...
  const filterProjects = projects => {
    ...
    return projects.filter(project => {
      const sizeFilter =
        filters.size.length === 0 || filters.size.includes(project.size)
      const durationFilter =
        filters.duration.length === 0 ||
        filters.duration.includes(project.duration.length)
